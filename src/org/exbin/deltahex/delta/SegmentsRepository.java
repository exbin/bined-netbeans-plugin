...
public class SegmentsRepository {
     ...
     * @param sourcePosition position of the shift
     * @param shift direction of the shift
     */
    private void shiftSegments(MemorySegment memorySegment, long sourcePosition, long shift) {
        MemoryDataSource source = memorySegment.getSource();
        DataSegmentsMap segmentsMap = memorySources.get(source);
        SegmentRecord record = segmentsMap.focusFirstOverlay(sourcePosition, source.getDataSize() - sourcePosition);
        while (record != null) {
            SegmentRecord nextRecord = record.getNext();
            if (record.dataSegment != memorySegment && record.getStartPosition() >= sourcePosition) {
