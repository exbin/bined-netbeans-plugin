...
@ActionID(
        category = "Tools",
        id = "org.exbin.bined.netbeans.diff.BinEdDiffAction"
)
@ActionRegistration(
        displayName = "#CTL_BinEdDiffAction"
)
public class BinEdDiffAction extends NodeAction {
    ...
    private class DiffActionImpl extends AbstractAction {
        ...
        private DiffActionImpl(Lookup context) {
            ...
            if (nodes.length == 1) {
                putValue(Action.NAME, NbBundle.getMessage(BinEdDiffAction.class, "CTL_DiffToActionName"));
            } else {
        ...
        public boolean isEnabled() {
            return BinEdDiffAction.this.enable(nodes);
        }
    }
    ...
    public String getName() {
        return NbBundle.getMessage(BinEdDiffAction.class, "CTL_DiffActionName");
    }
    ...
    private FileObject promptForFileobject(FileObject peer) {
        ...
        JFileChooser fileChooser = new AccessibleJFileChooser(NbBundle.getMessage(BinEdDiffAction.class, "ACSD_BrowseDiffToFile"), latestPath); // NOI18N
        fileChooser.setDialogTitle(NbBundle.getMessage(BinEdDiffAction.class, "DiffTo_BrowseFile_Title", peer.getName())); // NOI18N
        ...
        int result = fileChooser.showDialog(WindowManager.getDefault().getMainWindow(), NbBundle.getMessage(BinEdDiffAction.class, "DiffTo_BrowseFile_OK")); // NOI18N
    ...
    static void performAction(FileObject fo1, FileObject fo2, FileObject type) {
        ...
        if (diff == null) {
            DialogDisplayer.getDefault().notify(new NotifyDescriptor.Message(NbBundle.getMessage(BinEdDiffAction.class,
        ...
        try {
            ...
            String name = NbBundle.getMessage(BinEdDiffAction.class, "BK0001");
            ProgressHandle ph = null;
            try {
                ph = ProgressHandle.createHandle(name, killer);
                ph.start();
